...
for i, composer in enumerate(composers):
    print(f"[{i+1}/{len(composers)}] Patching {composer}...")
    
    try:
        # Step A (Find ID with Override support)
        if composer in TMDB_ID_OVERRIDES:
            person_id = TMDB_ID_OVERRIDES[composer]
            print(f"  Using override ID: {person_id}")
        else:
        ...
        # Step C (Filter - Broadened for patch)
        crew = credits_data.get("movie_credits", {}).get("crew", [])
        filtered_credits = [
            item for item in crew 
            if item.get("department") == "Sound" and item.get("job") in ["Original Music Composer", "Music", "Composer"]
        ]
